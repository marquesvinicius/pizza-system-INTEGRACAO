...
const authMiddleware = (req, res, next) => {
  ...
  const publicRoutes = ["/clients", "/products", "/orders"]; // Adicione mais conforme necessário

  // Se a rota estiver na lista de rotas públicas, permita o acesso sem autenticação
  ...
  if (parts.length !== 2 || parts[0] !== "Bearer") {
    return res
      .status(401)
      .json({ error: "Token mal formatado. Use o formato: Bearer <token>" });
  }
  ...
  try {
    ...
    return res
      .status(401)
      .json({ error: "Erro na autenticação", details: err.message });
  }
};
